...
class TrainingOrchestrator:
    ...
    def __init__(
        self, model: nn.Module, config: TrainingConfig, device: Optional[torch.device] = None
    ):
        """
    ...
    def _build_scheduler(self) -> torch.optim.lr_scheduler._LRScheduler:
        ...
        if scheduler_config.name.lower() == "cosine":
            scheduler = CosineAnnealingLR(
                self.optimizer, T_max=scheduler_config.max_steps, eta_min=scheduler_config.min_lr
            )
    ...
    def train_epoch(self, data_loader: DataLoader) -> Dict[str, float]:
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            if batch_idx % 10 == 0:
                ...
                current_lr = self.optimizer.param_groups[0]["lr"]
                pbar.set_postfix({"loss": f"{current_loss:.6f}", "lr": f"{current_lr:.2e}"})
        ...
        epoch_metrics = {"loss": np.mean(epoch_losses), "loss_std": np.std(epoch_losses)}
    ...
    def validate(self, data_loader: DataLoader) -> Dict[str, float]:
        ...
        val_metrics = {"loss": np.mean(val_losses), "loss_std": np.std(val_losses)}
