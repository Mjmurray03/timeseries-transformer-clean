...
class MultiHeadAttentionPooling(nn.Module):
    ...
    def __init__(
        self, d_model: int, n_heads: int = 8, dropout: float = 0.1, temperature: float = 1.0
    ):
        super().__init__()
...
class HierarchicalAttentionPooling(nn.Module):
    ...
    def forward(
        ...
        x_windows = x_padded.view(batch_size, n_windows, self.window_size, d_model)  # (B, W, S, D)

        if mask_padded is not None:
            mask_windows = mask_padded.view(batch_size, n_windows, self.window_size)  # (B, W, S)
        else:
            mask_windows = None
