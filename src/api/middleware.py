...
from .schemas import RateLimitInfo, AuthResponse
...
async def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)) -> AuthResponse:
...
async def get_optional_user(credentials: HTTPAuthorizationCredentials = Depends(security)) -> Optional[AuthResponse]:
...
def require_rate_limit_check(limit: Optional[int] = None):
    ...
    async def _check_rate_limit(request: Request, user: Optional[AuthResponse] = Depends(get_optional_user)):
