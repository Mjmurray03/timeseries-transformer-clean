...
class CostModel:
    ...
    def __init__(self, cost_config: Dict[str, Any]):
        ...
        self.slippage = cost_config.get('slippage', {
            'base': 0.0005, 
            'volatility_factor': 0.1, 
            'size_impact': 0.0001
        })
        self.market_impact = cost_config.get('market_impact', {
            'temporary': 0.0002, 
            'permanent': 0.0001
        })
    ...
    def calculate_slippage(self, ticker: str, shares: int, order_type: str, market_data: pd.Series) -> Dict[str, float]:
        """
...
class MarketSimulator:
    ...
    def _execute_single_order(self, signal, market_data: pd.Series, portfolio) -> Optional[ExecutedTrade]:
        ...
        slippage_info = self.cost_model.calculate_slippage(ticker, signal.shares, order_type, ticker_data)
        ...
        if order_type in ['BUY', 'LIMIT_BUY']:
            ...
            if required_capital > portfolio.cash:
                logger.warning(f"Insufficient cash for {ticker} purchase, required: ${required_capital:.2f}, available: ${portfolio.cash:.2f}")
                return None
        ...
        logger.debug(f"Executed {order_type} order: {executed_trade.shares} shares of {ticker} "
    ...
    def calculate_market_impact(self, ticker: str, shares: int, market_data: pd.Series) -> Dict[str, float]:
        """
    ...
    def get_execution_summary(self, executed_trades: List[ExecutedTrade]) -> Dict[str, Any]:
        ...
        return {
            ...
            'cost_as_pct_of_value': (total_commission + total_slippage + total_spread_cost) / total_value * 100,
