...
class ReportGenerator:
    ...
    def generate_full_report(
        self, backtest_results: Dict[str, Any], output_dir: str = "reports"
    ) -> Dict[str, str]:
        """
    ...
    def _create_summary_page(self, results: Dict[str, Any], pdf: PdfPages):
        ...
        colors = [
            self.colors["negative"] if x > 0.15 else self.colors["positive"]
            for x in risk_values[:2]
        ]
...
• Strategy: ML-based prediction strategy
    ...
    def _create_performance_page(self, results: Dict[str, Any], pdf: PdfPages):
        ...
        if "equity_curve" in results:
            ...
            ax1.plot(
                dates_parsed, values, color=self.colors["portfolio"], linewidth=2, label="Portfolio"
            )
        ...
        if "daily_returns" in results:
            ...
            ax3.hist(
                daily_returns, bins=50, alpha=0.7, color=self.colors["portfolio"], edgecolor="black"
            )
    ...
    def _create_risk_page(self, results: Dict[str, Any], pdf: PdfPages):
        ...
        if "daily_returns" in results:
            ...
            ax1.axvline(x=var_95, color="orange", linestyle="--", label=f"VaR 95%: {var_95:.2%}")
            ax1.axvline(x=var_99, color="red", linestyle="--", label=f"VaR 99%: {var_99:.2%}")
        ...
        if "equity_curve" in results:
            ...
            ax2.plot(
                rolling_vol.index, rolling_vol.values, color=self.colors["drawdown"], linewidth=2
            )
        ...
        ax4.axis("off")
        table = ax4.table(
            cellText=risk_data, colLabels=["Risk Metric", "Value"], cellLoc="center", loc="center"
        )
        table.auto_set_font_size(False)
...
• Commission as % of Volume: {(total_commission / trade_df['execution_price'].sum() * 100):.3f}%
    ...
    def _create_rolling_metrics_page(self, results: Dict[str, Any], pdf: PdfPages):
        ...
        if "daily_returns" in results:
            ...
            if len(returns_series) >= 60:
                rolling_sharpe = (
                    returns_series.rolling(60).mean()
                    / returns_series.rolling(60).std()
                    * np.sqrt(252)
                )
                ax1.plot(
            ...
            if len(returns_series) >= 60:
                ...
                ax2.fill_between(
                    rolling_dd.index, 0, rolling_dd.values, color=self.colors["drawdown"], alpha=0.7
                )
    ...
    def _generate_html_dashboard(self, results: Dict[str, Any], output_path: Path):
        ...
        html_content = html_template.format(
            metrics_cards=metrics_cards, javascript_code=javascript_code
        )
