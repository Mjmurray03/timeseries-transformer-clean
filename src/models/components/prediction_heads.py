    ...
    def __init__(
        ...
        for i in range(shared_layers):
            ...
            shared_layers_list.extend(
                [nn.Linear(current_dim, next_dim), nn.GELU(), nn.Dropout(dropout)]
            )
...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, input_dim: int, hidden_dim: int, dropout: float, activation: nn.Module):
        super().__init__()
...
class MultiTaskPredictionHead(nn.Module):
    ...
    def __init__(
        ...
        if task_weights is None:
            task_weights = {"price": 1.0, "volatility": 1.0, "quantiles": 1.0, "direction": 1.0}
...
def create_prediction_head(
    head_type: str = "multi_task", d_model: int = 256, forecast_horizon: int = 5, **kwargs
) -> nn.Module:
    """
