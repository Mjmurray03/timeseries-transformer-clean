...
def create_sample_data():
    ...
    data['High'] = np.maximum(
        data['Open'], 
        data['Close']
    ) * np.random.uniform(1.000, 1.015, 100)
    data['Low'] = np.minimum(
        data['Open'], 
        data['Close']
    ) * np.random.uniform(0.985, 1.000, 100)
    
    # Volume with some correlation to price changes
    base_volume = 1000000
    volume_multiplier = 1 + np.abs(returns) * 5  # Higher volume on big moves
    data['Volume'] = (base_volume * volume_multiplier * np.random.uniform(0.5, 2.0, 100)).astype(int)
...
def main():
    ...
    try:
        ...
        for data_type, type_stats in stats.items():
            print(f"   {data_type}: {type_stats['file_count']} files, "
