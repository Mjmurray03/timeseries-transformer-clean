...
class YahooFinanceCollector:
    ...
    def __init__(self, config: DataConfig):
        ...
        self.rate_limiter = RateLimiter(rate=self.source_config.rate_limit, period=1)  # per second
        ...
        logger.info(
            f"YahooFinanceCollector initialized with rate limit: {self.source_config.rate_limit}/sec"
        )
    ...
    async def collect_ticker(
        ...
        try:
            ...
            if not validation_result.is_valid:
                logger.error(
                    f"Data validation failed for {ticker}: {[str(issue) for issue in validation_result.issues]}"
                )
    ...
    def _clean_data(self, data: pd.DataFrame, ticker: str) -> pd.DataFrame:
        ...
        logger.debug(
            f"Cleaned data for {ticker}: {len(cleaned)} rows, {len(cleaned.columns)} columns"
        )
...
async def download_ticker(
    ticker: str, config: DataConfig, date_range: str = "full_history"
) -> Optional[pd.DataFrame]:
    """
...
async def download_ticker_set(
    ticker_set: str, config: DataConfig, date_range: str = "full_history"
) -> Dict[str, pd.DataFrame]:
    """
