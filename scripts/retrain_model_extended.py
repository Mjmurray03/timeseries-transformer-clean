...
def create_enhanced_features(df):
    ...
    close_prices = (
        features_df["Close"].squeeze()
        if hasattr(features_df["Close"], "squeeze")
        else features_df["Close"]
    )
    ...
    volume = (
        features_df["Volume"].squeeze()
        if hasattr(features_df["Volume"], "squeeze")
        else features_df["Volume"]
    )
...
def train_with_validation(
    model, train_data, val_data, epochs=20, lr=1e-3, batch_size=32, device="cuda"
):
    ...
    train_dataset = torch.utils.data.TensorDataset(
        torch.FloatTensor(train_data["features"]), torch.FloatTensor(train_data["targets"])
    )
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True)

    val_dataset = torch.utils.data.TensorDataset(
        torch.FloatTensor(val_data["features"]), torch.FloatTensor(val_data["targets"])
    )
...
def main():
    ...
    test_dataset = torch.utils.data.TensorDataset(
        torch.FloatTensor(test_data["features"]), torch.FloatTensor(test_data["targets"])
    )
    ...
    is_realistic = validate_predictions_realistic(torch.tensor(all_predictions), phase="TEST SET")

    # Calculate final metrics
    mse = np.mean((all_predictions - all_actuals) ** 2)
    rmse = np.sqrt(mse)
    directional_accuracy = np.mean(np.sign(all_predictions) == np.sign(all_actuals))
