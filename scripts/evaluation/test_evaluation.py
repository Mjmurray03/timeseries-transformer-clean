...
def create_mock_model_checkpoint(temp_dir: Path) -> Path:
    ...
    checkpoint = {
        ...
        "epoch": 50,
        "metrics": {"val_rmse": 0.255, "val_r2": 0.67, "val_direction_accuracy": 0.58},
...
def create_mock_stock_data(temp_dir: Path, n_days: int = 500) -> Path:
    ...
    logging.info(
        f"Created mock data: {len(df)} days, price range ${df['Close'].min():.2f}-${df['Close'].max():.2f}"
    )
...
def test_sliding_window_predictions():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        predictions_df = pipeline.sliding_window_predictions(
            data=data, start_date="2023-03-01", end_date="2023-06-01", seq_len=60, stride=5
        )
        ...
        logging.info(
            f"✓ Current price range: ${current_prices.min():.2f}-${current_prices.max():.2f}"
        )
        logging.info(
            f"✓ Predicted price range: ${predicted_prices.min():.2f}-${predicted_prices.max():.2f}"
        )
...
def test_metrics_calculator():
    ...
    logging.info(
        f"✓ Regression metrics: RMSE={reg_metrics['rmse']:.4f}, R²={reg_metrics['r2']:.3f}"
    )
    logging.info(f"✓ Directional accuracy: {dir_metrics['overall_accuracy']:.1%}")
    logging.info(
        f"✓ Trading metrics: Sharpe={trading_metrics['sharpe_ratio']:.2f}, Win Rate={trading_metrics['win_rate']:.1%}"
    )
...
def test_end_to_end_evaluation():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        from evaluate import generate_comprehensive_report, load_stock_data, setup_logging
        ...
        pipeline = PredictionPipeline(model, scaler, torch.device("cpu"), config)
        predictions_df = pipeline.sliding_window_predictions(
            data=data, start_date="2023-03-01", end_date="2023-08-01", seq_len=60, stride=3
        )
        ...
        assert (
            0 <= report["directional_metrics"]["overall_accuracy"] <= 1
        ), "Invalid accuracy in report"
