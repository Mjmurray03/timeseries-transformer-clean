...
class Position:
    ...
    def close(self, exit_price: float, exit_date: datetime, commission: float = 0.0) -> float:
        ...
        self.realized_pnl = (exit_price - self.entry_price) * self.shares - self.commission - commission
...
class Portfolio:
    ...
    def _open_position(self, trade, market_prices: pd.Series):
        ...
        logger.debug(f"Opened position: {trade.shares} shares of {trade.ticker} at ${trade.execution_price:.2f}")
    ...
    def _close_position(self, trade, market_prices: pd.Series):
        ...
        logger.debug(f"Closed position: {position.shares} shares of {trade.ticker} "
                ...
                if isinstance(market_prices.index, pd.MultiIndex):
                    ...
                    if hasattr(market_prices.loc[ticker], 'close'):
                        ...
                        if isinstance(price_data, pd.Series):
                            position.update_price(price_data.iloc[0] if len(price_data) > 0 else position.current_price)
                        else:
    ...
    def realized_pnl(self) -> float:
        ...
        return sum(pos.realized_pnl for pos in self.closed_positions if pos.realized_pnl is not None)
    ...
    def calculate_portfolio_risk(self, returns_data: pd.DataFrame, lookback_days: int = 30) -> float:
        """
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'sharpe_ratio': (np.mean(self.daily_returns) / np.std(self.daily_returns) * np.sqrt(252)) 
                           if self.daily_returns and np.std(self.daily_returns) > 0 else 0.0
        }