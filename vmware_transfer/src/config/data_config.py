...
class StorageConfig:
    ...
    raw_data: Dict[str, str] = field(default_factory=lambda: {
        "format": "parquet",
        "compression": "snappy",
        "partition_by": "ticker"
    })
...
class DataConfig:
    ...
    def __init__(self, config_dict: Dict[str, Any]):
        ...
        self.parallel_processing = ParallelProcessingConfig(**config_dict.get("parallel_processing", {}))
    ...
    def _parse_features_config(self, features_config: Dict[str, Any]):
        ...
        for name, indicator_config in tech_indicators.items():
            if isinstance(indicator_config, dict):
                self.features.technical_indicators[name] = TechnicalIndicatorConfig(**indicator_config)
