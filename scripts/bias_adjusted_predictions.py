...
class BiasAdjustedPredictor:
    ...
    def prepare_features(self, ticker, lookback_days=100):
        ...
        df['RSI'] = returns_series.rolling(14).apply(
            lambda x: 100 - 100/(1 + (x[x>0].mean() / -x[x<0].mean())) if len(x[x<0]) > 0 else 50
        ).values
        ...
        feature_cols = [
            ...
            'returns', 'returns', 'volatility_20d',  # Using returns twice as placeholder
        ...
        for ticker in tickers:
            try:
                ...
                if pred:
                    ...
                    print(f"{ticker}: {pred['composite_signal']} "
