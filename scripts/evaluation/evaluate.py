...
class ModelLoader:
    ...
    def load_checkpoint(
        self, model_path: Path, scaler_path: Optional[Path] = None
...
class PredictionPipeline:
    ...
    def __init__(self, model: nn.Module, scaler: Dict, device: torch.device, config: Dict):
        self.model = model
        self.scaler = scaler
        self.device = device
        self.config = config
    ...
    def predict_sequence(
        ...
        expected_features = (
            len(self.feature_names) if self.feature_names else self.config["input_dim"]
        )
        ...
        with torch.no_grad():
            if return_attention:
                predictions, attention_weights = self.model(input_tensor, return_attention=True)
            else:
    ...
    def sliding_window_predictions(
        ...
        if len(available_features) != len(self.feature_names):
            logging.warning(
                f"Feature mismatch: expected {len(self.feature_names)}, got {len(available_features)}"
            )
...
class MetricsCalculator:
    ...
    def calculate_regression_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
        ...
        if len(y_true_clean) == 0:
            return {"rmse": np.nan, "mae": np.nan, "mape": np.nan, "r2": np.nan, "n_samples": 0}
    ...
    def calculate_trading_metrics(
        ...
        sharpe_ratio = (excess_returns.mean() / (excess_returns.std() + 1e-8)) * np.sqrt(
            annual_factor
        )
...
class EvaluationVisualizer:
    ...
    def plot_predictions_vs_actuals(
        self, df: pd.DataFrame, ticker: str = "Stock", save_path: Optional[Path] = None
    ):
        """
        ...
        if "actual_return_1d" in df.columns:
            ...
            if "predicted_1d" in df.columns and "actual_1d" in df.columns:
                for ax, metric in zip(
                    axes, ["RMSE", "Directional Accuracy", "Returns Correlation"]
                ):
                    regime_metrics = []
                    regime_names = []
                    ...
                    for condition in ["Low Vol", "Med Vol", "High Vol"]:
                        ...
                        if len(condition_data) > 10:  # Minimum samples
                            if metric == "RMSE":
                                ...
                                if len(common_idx) > 0:
                                    rmse = np.sqrt(
                                        mean_squared_error(actual[common_idx], pred[common_idx])
                                    )
                                ...
                                if len(common_idx) > 0:
                                    dir_acc = MetricsCalculator.calculate_directional_accuracy(
                                        actual_ret[common_idx].values, pred_ret[common_idx].values
                                    )["overall_accuracy"]
                                ...
                                if len(common_idx) > 1:
                                    corr = np.corrcoef(
                                        actual_ret[common_idx], pred_ret[common_idx]
                                    )[0, 1]
                    ...
                    if regime_metrics:
                        bars = ax.bar(
                            regime_names, regime_metrics, alpha=0.7, color=["blue", "orange", "red"]
                        )
    ...
    def plot_error_analysis(self, df: pd.DataFrame, save_path: Optional[Path] = None):
        """
...
def generate_comprehensive_report(
    ...
    if "rmse" in regression_metrics:
        ...
        summary["baseline_comparison"] = {
            ...
            "improvement": (
                (0.268 - regression_metrics["rmse"]) / 0.268
                if regression_metrics["rmse"] < 0.268
                else 0
            ),
        }
...
def main():
    ...
    parser.add_argument(
        "--data-path", type=str, required=True, help="Path to data file or directory"
    )
    parser.add_argument("--scaler-path", type=str, help="Path to scaler JSON file (optional)")
    parser.add_argument(
        "--ticker",
        type=str,
        default="AAPL",
    ...
    parser.add_argument(
        "--end-date", type=str, default="2023-12-31", help="End date for evaluation (YYYY-MM-DD)"
    )
    parser.add_argument(
        "--output-dir", type=str, default="evaluation_results", help="Output directory for results"
    )
    parser.add_argument("--seq-len", type=int, default=60, help="Sequence length for model input")
    parser.add_argument(
        "--stride", type=int, default=1, help="Stride for sliding window predictions"
    )
    parser.add_argument(
        "--device", type=str, default="auto", help="Device for model inference (auto, cpu, cuda)"
    )
    parser.add_argument(
        "--risk-free-rate",
        type=float,
    ...
    try:
        ...
        # Directional metrics
        if (
            "actual_return_1d" in predictions_df.columns
            and "predicted_return_1d" in predictions_df.columns
        ):
        ...
        try:
            ...
            if sample_idx + args.seq_len < len(data):
                ...
                sample_features = np.column_stack(
                    [sample_features, np.zeros(len(sample_features))]  # Add returns placeholder
                )
