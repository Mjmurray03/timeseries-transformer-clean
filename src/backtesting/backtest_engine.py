...
class BacktestEngine:
    ...
    def __init__(self, config: BacktestConfig):
        ...
        logger.info(
            f"BacktestEngine initialized with ${config.initial_capital:,.2f} initial capital"
        )
    ...
    def _process_trading_day(
        self, date: pd.Timestamp, predictions: pd.DataFrame, market_data: pd.DataFrame
    ):
        ...
        try:
            ...
            approved_signals = self.risk_manager.filter_signals(
                signals, self.portfolio, daily_market_data
            )

            if len(approved_signals) < len(signals):
                logger.debug(
                    f"{date}: Risk manager filtered {len(signals) - len(approved_signals)} signals"
                )

            # Execute trades
            executed_trades = self.market_sim.execute_orders(
                approved_signals, daily_market_data, self.portfolio
            )
            ...
            portfolio_snapshot = {
                ...
                "daily_return": (
                    self.portfolio.daily_return if hasattr(self.portfolio, "daily_return") else 0.0
                ),
                "positions": {
                    ticker: {
                        "shares": pos.shares,
    ...
    def run_walk_forward_analysis(
        ...
        """
        logger.info(
            f"Starting walk-forward analysis with {train_window}d train, {test_window}d test windows"
        )
        ...
        all_sharpe = [
            r["metrics"]["sharpe_ratio"] for r in results if r["metrics"]["sharpe_ratio"] != 0
        ]
