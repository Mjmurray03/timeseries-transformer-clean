...
def create_experiment_group(
    group_name: str, experiments: List[str], description: Optional[str] = None
) -> None:
    """
...
def init_wandb_for_evaluation(
    model_version: str, test_config: Dict[str, Any], experiment_name: Optional[str] = None
) -> wandb.sdk.wandb_run.Run:
    """Initialize WANDB for model evaluation."""
    tags = ["evaluation", "testing", model_version]

    config = {"model_version": model_version, "evaluation": test_config, "job_type": "evaluation"}
    ...
    return init_wandb(
        ...
        job_type="evaluation",
        experiment_name=experiment_name
        or f"eval_{model_version}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
    )


def init_wandb_for_hyperparameter_search(
    search_config: Dict[str, Any], experiment_name: Optional[str] = None
) -> wandb.sdk.wandb_run.Run:
    """Initialize WANDB for hyperparameter optimization."""
    tags = ["hyperparameter_search", "optimization", "sweep"]

    config = {"search_space": search_config, "job_type": "hyperparameter_search"}
