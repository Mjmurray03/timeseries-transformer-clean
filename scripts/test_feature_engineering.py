...
def create_sample_data():
    ...
    data['High'] = np.maximum(
        data['Open'], 
        data['Close']
    ) * np.random.uniform(1.000, 1.015, 200)
    data['Low'] = np.minimum(
        data['Open'], 
        data['Close']
    ) * np.random.uniform(0.985, 1.000, 200)
    
    # Volume with some correlation to price changes
    base_volume = 1000000
    volume_multiplier = 1 + np.abs(returns) * 5  # Higher volume on big moves
    data['Volume'] = (base_volume * volume_multiplier * np.random.uniform(0.5, 2.0, 200)).astype(int)
...
def main():
    ...
    # Price features
    price_features = [col for col in result.columns if col in ['Returns', 'LogReturns', 'HL_Ratio', 'OC_Ratio', 'Gap']]
    print(f"   Price features ({len(price_features)}): {price_features}")
    
    # Technical indicators
    tech_features = [col for col in result.columns if col in ['RSI', 'MACD', 'MACD_Signal', 'MACD_Histogram']]
    ...
    for col in key_indicators:
        if col in sample_data.columns:
            ...
            print(f"   {col:12}: {' '.join([f'{v:8.4f}' if not pd.isna(v) else '     NaN' for v in values])}")
    print()
    ...
    for feature in ['RSI', 'MACD', 'BB_Width', 'Volume_Ratio', 'Volatility']:
        if feature in summary:
            ...
            print(f"   {feature:12}: mean={stats['mean']:8.4f}, std={stats['std']:8.4f}, "
    ...
    print(f"   MACD: range=[{macd_valid.min():.4f}, {macd_valid.max():.4f}], "
    ...
    print(f"   BB Width: range=[{bb_width_valid.min():.4f}, {bb_width_valid.max():.4f}], "
