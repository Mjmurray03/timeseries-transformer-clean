...
class BacktestReport:
    ...
    def _calculate_performance_metrics(self) -> Dict[str, float]:
        ...
        if len(benchmark_returns) > 0:
            ...
            benchmark_sharpe = (
                benchmark_total * self.trading_days / len(benchmark_returns)
            ) / benchmark_vol
            ...
            alpha = annualized_return - (
                beta * benchmark_total * self.trading_days / len(benchmark_returns)
            )
            information_ratio = (
                (annualized_return - benchmark_total * self.trading_days / len(benchmark_returns))
    ...
    def _calculate_trading_metrics(self) -> Dict[str, Any]:
        ...
        if "pnl" in trades.columns:
            ...
            if "entry_price" in trades.columns and "exit_price" in trades.columns:
                trade_returns = (trades["exit_price"] - trades["entry_price"]) / trades[
                    "entry_price"
                ]
            else:
    ...
    def _create_tearsheet(self, save_path: str):
        ...
        if len(self.results.benchmark_returns) > 0:
            benchmark_values = (
                1 + self.results.benchmark_returns
            ).cumprod() * self.results.initial_capital
        ...
        ax5.axvline(
            x=self.results.returns.mean(),
            color="green",
            linestyle="--",
            label=f"Mean: {self.results.returns.mean():.3f}",
        )
        ax5.legend()
        ...
        if hasattr(self.results, "rolling_volatility") and len(self.results.rolling_volatility) > 0:
            (self.results.rolling_volatility * np.sqrt(252)).plot(
                ax=ax6, linewidth=2, color="orange"
            )
        ...
        if len(self.results.trades) > 0:
            trade_returns = self.results.trades.get(
                "pnl", self.results.trades.get("return", pd.Series())
            )
    ...
    def _create_executive_summary(
        self, performance: Dict, risk: Dict, trading: Dict
    ) -> Dict[str, Any]:
...
def create_sample_backtest_results() -> BacktestResults:
    ...
    # Sample positions
    positions = pd.DataFrame(
        {"total_exposure": np.random.uniform(50000, 95000, len(dates))}, index=dates
    )
