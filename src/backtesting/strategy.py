...
class Strategy:
    ...
    def __init__(self, strategy_params: Dict[str, Any]):
        ...
        logger.info(f"Strategy initialized: min_return={self.min_expected_return:.2%}, "
                   f"min_confidence={self.min_confidence:.2%}, max_positions={self.max_positions}")
    
    def generate_signals(self, predictions: pd.Series, positions: Dict, historical_data: pd.DataFrame) -> List[TradingSignal]:
        """
        ...
        for ticker in predictions.index:
            try:
                ...
                if self.should_enter(ticker, prediction, positions):
                    ...
                    logger.debug(f"Entry signal: {signal.type} {signal.shares} shares of {ticker} "
                ...
                # Check exit conditions for existing positions
                    ...
                    if self.should_exit(ticker, positions[ticker], prediction):
                        ...
                        signals.append(signal)
                        logger.debug(f"Exit signal: {signal.type} {signal.shares} shares of {ticker}")
    ...
    def _parse_prediction(self, prediction_data) -> Dict[str, float]:
        ...
        if isinstance(prediction_data, dict):
            ...
            return {
                'return_5d': prediction_data.get('return_5d', prediction_data.iloc[0] if len(prediction_data) > 0 else 0.0),
    ...
    def backtest_signal_performance(self, signals: List[TradingSignal], 
                                   actual_returns: pd.DataFrame) -> Dict[str, Any]:
        """
