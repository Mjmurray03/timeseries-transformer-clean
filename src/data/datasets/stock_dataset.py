...
class StockSequenceDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        return {"inputs": sequence, "targets": target, "index": torch.tensor(idx, dtype=torch.long)}
...
class MultiStockDataset(Dataset):
    ...
    def __init__(
        ...
        logger.info(
            f"Created multi-stock dataset with {len(self)} samples from {len(stock_data)} stocks"
        )
