...
class TemporalMaskGenerator:
    ...
    def create_causal_mask(
        seq_len: int, device: Optional[torch.device] = None, dtype: torch.dtype = torch.bool
    ) -> torch.Tensor:
        """
    ...
    def create_padding_mask(
        lengths: torch.Tensor, max_len: Optional[int] = None, device: Optional[torch.device] = None
    ) -> torch.Tensor:
        """
...
class AdaptiveTemporalMask(nn.Module):
    ...
    def __init__(
        self, seq_len: int, d_model: int, mask_ratio: float = 0.1, temperature: float = 1.0
    ):
        super().__init__()
        ...
        self.register_buffer("causal_mask", TemporalMaskGenerator.create_causal_mask(seq_len))

    def forward(
        self, x: torch.Tensor, base_mask: Optional[torch.Tensor] = None, training: bool = True
    ) -> torch.Tensor:
        """
...
class TemporalMaskingLayer(nn.Module):
    ...
    def __init__(
        ...
        # Create base masks
        if mask_type == "causal":
            self.register_buffer("base_mask", TemporalMaskGenerator.create_causal_mask(seq_len))
        elif mask_type == "sliding_window":
            if window_size is None:
                window_size = min(32, seq_len // 2)
            self.register_buffer(
                "base_mask", TemporalMaskGenerator.create_sliding_window_mask(seq_len, window_size)
            )
        elif mask_type == "block_diagonal":
            block_size = window_size or 16
            self.register_buffer(
                "base_mask", TemporalMaskGenerator.create_block_diagonal_mask(seq_len, block_size)
            )
        else:
...
class FutureLeakageDetector:
    ...
    def detect_leakage(
        self, attention_weights: torch.Tensor, causal_mask: Optional[torch.Tensor] = None
    ) -> dict:
        """
        ...
        leakage_info = {
            ...
            "leakage_ratio": (
                total_future_attention.sum(dim=-1) / attention_weights.sum(dim=(-2, -1))
            )
            .mean()
            .item(),
    ...
    def validate_causal_attention(
        self, attention_weights: torch.Tensor, strict: bool = True
    ) -> bool:
        """
