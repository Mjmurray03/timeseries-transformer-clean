...
def test_single_ticker_approach(ticker: str = "AAPL") -> Dict[str, any]:
    ...
    results = {"approach": "single_ticker", "ticker": ticker, "success": False, "error": None}
...
def test_multi_stock_approach() -> Dict[str, any]:
    ...
    results = {"approach": "multi_stock", "success": False, "error": None}
...
def main():
    ...
    if multi_results["success"] and successful_single > 0:
        ...
        if single_rmse_values:
            ...
            logging.info(
                f"Best single-ticker RMSE: {best_single:.6f} vs baseline {baseline_rmse} {vs_baseline}"
            )
        ...
        if multi_rmse_values:
            ...
            logging.info(
                f"Best multi-stock RMSE: {best_multi:.6f} vs baseline {baseline_rmse} {vs_baseline}"
            )
