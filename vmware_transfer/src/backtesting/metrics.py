    ...
    def update(self, portfolio, date: datetime):
        ...
        try:
            ...
            if self.portfolio_values:
                daily_return = (current_value - self.portfolio_values[-1]) / self.portfolio_values[-1]
            else:
                daily_return = 0.0
...
class PerformanceAnalyzer:
    ...
    def calculate_metrics(self, portfolio_history: List[Dict], closed_positions: List = None) -> Dict[str, Any]:
        """
        ...
        # Trading metrics
        trading_metrics = self._calculate_trading_metrics(closed_positions) if closed_positions else {}
    ...
    def _calculate_advanced_metrics(self, returns: pd.Series) -> Dict[str, float]:
        ...
        loss_95 = abs(np.percentile(returns[returns < 0], 5)) if len(returns[returns < 0]) > 0 else 1
    ...
    def calculate_rolling_metrics(self, returns: pd.Series, window: int = 252) -> Dict[str, pd.Series]:
    ...
    def compare_to_benchmark(self, returns: pd.Series, benchmark_returns: pd.Series) -> Dict[str, float]:
        ...
        # Alpha calculation
        alpha = port_returns.mean() * 252 - (self.risk_free_rate + beta * (bench_returns.mean() * 252 - self.risk_free_rate))
