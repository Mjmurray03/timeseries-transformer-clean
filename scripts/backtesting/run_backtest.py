...
def create_backtest_results_from_engine(results: dict, config: BacktestConfig) -> BacktestResults:
    ...
    try:
        ...
        if "portfolio_history" in results and results["portfolio_history"]:
            ...
            if "portfolio_value" in portfolio_df.columns:
                ...
                portfolio_values = pd.Series(
                    [config.initial_capital] * len(portfolio_df), index=portfolio_df.index
                )
            ...
            positions = portfolio_df[
                ["cash", "total_value"]
                + [col for col in portfolio_df.columns if col.endswith("_value")]
            ]
            ...
            portfolio_values = pd.Series(
                [config.initial_capital * (1 + daily_return) ** i for i in range(len(date_range))],
                index=date_range,
            )
        ...
        benchmark_returns = pd.Series(
            np.random.normal(0.0005, 0.012, len(portfolio_values)), index=portfolio_values.index
        )
...
def run_walk_forward_analysis(
    ...
    while current_start + test_window < total_days:
        ...
        logging.info(
            f"Walk-forward period {period_count}: {test_start_date.date()} to {test_end_date.date()}"
        )
        ...
        try:
            ...
            logging.info(
                f"Period {period_count} complete: "
    ...
    if successful_periods:
        ...
        aggregate_metrics = {
            ...
            "consistency": (
                np.std(returns) / abs(np.mean(returns)) if np.mean(returns) != 0 else float("inf")
            ),
        }
    else:
    ...
    # Save results
    results_file = (
        output_dir / f"walk_forward_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
