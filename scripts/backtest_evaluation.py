    ...
    def backtest(self, tickers, start_date, end_date, rebalance_freq="3D"):
        ...
        for ticker in data:
            if first_valid_date in data[ticker].index:
                ...
                buy_hold_strategy.execute_trade(ticker, shares, initial_price, first_valid_date)

        # Prepare for backtesting loop
        trading_days = pd.date_range(start=first_valid_date, end=end_date, freq="B")
        ...
        for date in trading_days:
            ...
            if date in rebalance_dates:
                        ...
                        if ticker in current_prices:
                            ...
                            if shares != 0:
                                model_strategy.execute_trade(
                                    ticker, -shares, current_prices[ticker], date
                                )
                ...
                # Random positions
                random_tickers = np.random.choice(
                    list(current_prices.keys()), size=min(3, len(current_prices)), replace=False
                )
                ...
                for ticker in random_tickers:
                    ...
                    if np.random.random() > 0.5:  # Random long/short
                        shares = -shares
                    random_strategy.execute_trade(ticker, shares, current_prices[ticker], date)
