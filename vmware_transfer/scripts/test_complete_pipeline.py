...
class CompleteTestResult:
    ...
    def add_stage_result(self, stage_name: str, success: bool, duration: float, 
                        metrics: Dict[str, Any], error: Optional[str] = None):
...
class EndToEndPipelineTest:
    ...
    def run_complete_test(self) -> CompleteTestResult:
        ...
        try:
            ...
            training_results = self._test_training_orchestrator_stage(
            ...
            self._test_verification_stage(raw_data, feature_data, datasets, 
                                        model, config, training_results)
    ...
    def _test_data_loading_stage(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            metrics = {
                ...
                'date_range': {
                    'start': str(raw_data.index.min()) if hasattr(raw_data.index, 'min') else 'unknown',
                    'end': str(raw_data.index.max()) if hasattr(raw_data.index, 'max') else 'unknown'
                },
    ...
    def _test_feature_engineering_stage(self, raw_data: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            found_features = [feat for feat in expected_features if any(feat in col for col in feature_data.columns)]
    ...
    def _test_dataset_creation_stage(self, feature_data: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if 'close' in clean_data.columns:
                ...
                raise ValueError(f"No 'close' or 'Close' column found in cleaned data. Available: {list(clean_data.columns)}")
            ...
            print(f"+ Train/Val/Test Split: {metrics['train_size']}/{metrics['val_size']}/{metrics['test_size']}")
    ...
    def _test_model_initialization_stage(self, feature_data: pd.DataFrame) -> Optional[TimeSeriesTransformer]:
    ...
    def _test_training_orchestrator_stage(self, model: TimeSeriesTransformer, 
                                        config: TrainingConfig, 
                                        datasets: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for batch_idx, batch in enumerate(train_loader):
                ...
                print(f"  Batch {batch_idx + 1}/{min(3, len(train_loader))}, Loss: {loss.item():.6f}")
            ...
            with torch.no_grad():
                for batch_idx, batch in enumerate(val_loader):
                    ...
                    print(f"  Val Batch {batch_idx + 1}/{min(2, len(val_loader))}, Loss: {loss.item():.6f}")
    ...
    def _test_verification_stage(self, raw_data: pd.DataFrame, feature_data: pd.DataFrame,
                               datasets: Dict[str, Any], model: TimeSeriesTransformer,
                               config: TrainingConfig, training_results: Dict[str, Any]):
        ...
        try:
            ...
            losses_reasonable = (
                0 < training_results['avg_train_loss'] < 1000 and
                0 < training_results['avg_val_loss'] < 1000
            )
            ...
            metrics = {
                ...
                'verification_summary': {
                    ...
                    'shape_consistency': 'PASS' if all(verifications['shape_consistency'].values()) else 'FAIL',
                    'type_consistency': 'PASS' if all(verifications['type_consistency'].values()) else 'FAIL',
                    'loss_verification': 'PASS' if all(verifications['loss_verification'].values()) else 'FAIL'
                }
            }
