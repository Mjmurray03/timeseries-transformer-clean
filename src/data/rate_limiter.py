...
class MultiRateLimiter:
    ...
    def add_limiter(
        self, name: str, rate: int, period: int = 1, burst_size: Optional[int] = None
    ) -> RateLimiter:
        """
    ...
    def get_statistics(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {name: limiter.get_statistics() for name, limiter in self.limiters.items()}
...
def create_rate_limiter_from_config(config: RateLimitConfig) -> RateLimiter:
    ...
    """
    return RateLimiter(rate=config.rate, period=config.period, burst_size=config.burst_size)
