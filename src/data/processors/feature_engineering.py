...
class FeatureEngineer:
    ...
    def _get_default_config(self) -> Dict:
        ...
        return {
            ...
            "moving_averages": {"enabled": True, "periods": [5, 10, 20, 50, 200]},
    ...
    def engineer_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.config["rsi"]["enabled"]:
            result["RSI"] = self.calculate_rsi(result["Close"], period=self.config["rsi"]["period"])
        ...
        logger.info(
            f"Feature engineering complete. Added {len(result.columns) - len(data.columns)} features"
        )
    ...
    def calculate_macd(
        ...
        return {"MACD": macd, "MACD_Signal": macd_signal, "MACD_Histogram": macd_histogram}
    ...
    def validate_features(self, data: pd.DataFrame) -> bool:
        ...
        try:
            ...
            if all(col in data.columns for col in ["BB_Upper", "BB_Middle", "BB_Lower"]):
                valid_bb_data = ~(
                    data["BB_Upper"].isna() | data["BB_Middle"].isna() | data["BB_Lower"].isna()
                )
                if valid_bb_data.any():
                    bb_valid = (
                        data.loc[valid_bb_data, "BB_Upper"] >= data.loc[valid_bb_data, "BB_Middle"]
                    ).all() and (
                        data.loc[valid_bb_data, "BB_Middle"] >= data.loc[valid_bb_data, "BB_Lower"]
                    ).all()
                    if not bb_valid:
