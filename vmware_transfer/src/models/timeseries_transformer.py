...
class TimeSeriesTransformer(nn.Module):
    ...
    def count_parameters(self) -> Dict[str, int]:
        ...
        component_params = {}
        component_params['input_embedding'] = sum(p.numel() for p in self.input_embedding.parameters())
        component_params['positional_encoding'] = sum(p.numel() for p in self.positional_encoding.parameters())
        component_params['transformer_layers'] = sum(p.numel() for p in self.transformer_layers.parameters())
        if self.attention_pooling:
            component_params['attention_pooling'] = sum(p.numel() for p in self.attention_pooling.parameters())
        component_params['output_layer'] = sum(p.numel() for p in self.output_layer.parameters())
        component_params['multi_task_heads'] = sum(p.numel() for p in self.multi_task_heads.parameters())
