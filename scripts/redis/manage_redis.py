    ...
    def _is_redis_running_docker(self) -> bool:
        ...
        try:
            result = self._run_command([
                "docker", "ps", "--filter", "name=timeseries-redis", 
                "--format", "{{.Names}}"
            ], check=False)
    ...
    def deploy_configuration(self) -> bool:
        ...
        try:
            ...
            if self.platform == "linux":
                try:
                    self._run_command([
                        "sudo", "cp", self.config_path, "/etc/redis/redis.conf"
                    ])
                ...
                try:
                    self._run_command([
                        "cp", self.config_path, "/usr/local/etc/redis.conf"
                    ])
...
def main():
    ...
    parser.add_argument(
        "--config",
        help="Path to Redis configuration file"
    )
