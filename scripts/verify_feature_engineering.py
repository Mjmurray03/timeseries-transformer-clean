...
class FeatureVerifier:
    ...
    def verify_all_features(self, data_path: str = None) -> Dict:
        ...
        print(f"Engineered features: {engineered_df.shape[0]} rows × {engineered_df.shape[1]} columns")
    ...
    def _verify_dimensions(self, input_df: pd.DataFrame, output_df: pd.DataFrame):
        ...
        if output_shape[0] > input_shape[0]:
            self._report_error("DIMENSIONS", f"Output rows ({output_shape[0]}) > input rows ({input_shape[0]})")
            return
            
        if output_shape[1] < 30:  # Should have at least 30 features
            self._report_error("DIMENSIONS", f"Output features expected >= 30, got {output_shape[1]}")
            return
    ...
    def _verify_sma(self, input_df: pd.DataFrame, output_df: pd.DataFrame, sample_indices: List):
        ...
        for idx in sample_indices:
            ...
            if diff > self.tolerance:
                self._report_error("SMA_20", f"At {idx.date()}: Expected {expected_sma:.6f}, got {actual_val:.6f}, diff {diff:.6f}")
                return
    ...
    def _verify_rsi(self, input_df: pd.DataFrame, output_df: pd.DataFrame, sample_indices: List):
        ...
        if not rsi_out_of_range.empty:
            self._report_error("RSI", f"RSI values outside [0, 100] range: {len(rsi_out_of_range)} values")
            return
        ...
        for idx in sample_indices:
            ...
            if diff > self.tolerance:
                self._report_error("RSI", f"At {idx.date()}: Expected {expected_rsi:.6f}, got {actual_val:.6f}, diff {diff:.6f}")
                return
    ...
    def _verify_macd(self, input_df: pd.DataFrame, output_df: pd.DataFrame, sample_indices: List):
        ...
        for idx in sample_indices:
            ...
            for component in ['MACD', 'MACD_Signal', 'MACD_Histogram']:
                ...
                if diff > self.tolerance:
                    self._report_error("MACD", f"{component} at {idx.date()}: Expected {expected_val:.6f}, got {actual_val:.6f}, diff {diff:.6f}")
                    return
        
        self.results['MACD'] = 'PASS'
        print("+ MACD verification passed")
    
    def _calculate_manual_macd(self, prices: pd.Series, target_idx, fast=12, slow=26, signal=9) -> Dict:
    ...
    def _verify_bollinger_bands(self, input_df: pd.DataFrame, output_df: pd.DataFrame, sample_indices: List):
        ...
        for idx in sample_indices:
            ...
            expected_bb = self._calculate_manual_bollinger_bands(close_prices, idx, period=20, std_dev=2.0)
            if not expected_bb:
                continue
            ...
            for component in ['BB_Upper', 'BB_Middle', 'BB_Lower']:
                ...
                if diff > self.tolerance:
                    self._report_error("BOLLINGER_BANDS", f"{component} at {idx.date()}: Expected {expected_val:.6f}, got {actual_val:.6f}, diff {diff:.6f}")
                    return
        
        # Verify BB ordering (Upper >= Middle >= Lower)
        valid_bb_rows = ~(output_df['BB_Upper'].isna() | output_df['BB_Middle'].isna() | output_df['BB_Lower'].isna())
        if valid_bb_rows.any():
            upper_ge_middle = (output_df.loc[valid_bb_rows, 'BB_Upper'] >= output_df.loc[valid_bb_rows, 'BB_Middle']).all()
            middle_ge_lower = (output_df.loc[valid_bb_rows, 'BB_Middle'] >= output_df.loc[valid_bb_rows, 'BB_Lower']).all()
            
            if not upper_ge_middle or not middle_ge_lower:
                self._report_error("BOLLINGER_BANDS", "Bollinger Band ordering violated (Upper >= Middle >= Lower)")
                return
        
        self.results['BOLLINGER_BANDS'] = 'PASS'
        print("+ Bollinger Bands verification passed")
    
    def _calculate_manual_bollinger_bands(self, prices: pd.Series, target_idx, period: int = 20, std_dev: float = 2.0) -> Dict:
    ...
    def _verify_returns(self, input_df: pd.DataFrame, output_df: pd.DataFrame, sample_indices: List):
        ...
        for idx in sample_indices:
            ...
            if diff > self.tolerance:
                self._report_error("RETURNS", f"At {idx.date()}: Expected {expected_return:.6f}, got {actual_val:.6f}, diff {diff:.6f}")
                return
    ...
    def _verify_no_data_leakage(self, input_df: pd.DataFrame, output_df: pd.DataFrame):
        ...
        for indicator, expected_nan_count in indicators_with_lookback.items():
            if indicator in output_df.columns:
                ...
                if actual_nan_count < expected_nan_count * 0.8:  # Allow some flexibility
                    self._report_error("DATA_LEAKAGE", f"{indicator} should have ~{expected_nan_count} initial NaN values, got {actual_nan_count}")
                    return
        ...
        if 'RSI' in output_df.columns:
            ...
            if not (initial_rsi_values == 50).all():
                ...
                self._report_error("DATA_LEAKAGE", f"RSI first 14 values should be 50 (initialization), but {non_50_count} are different")
                return
        ...
        if 'Returns' in output_df.columns:
            if not pd.isna(output_df['Returns'].iloc[0]):
                self._report_error("DATA_LEAKAGE", "Returns first value should be NaN (no previous price to compare)")
                return
    ...
    def _verify_feature_completeness(self, output_df: pd.DataFrame):
        ...
        expected_features = [
            ...
            'BB_Upper', 'BB_Middle', 'BB_Lower', 'MA_20'
        ]
        
        missing_features = [f for f in expected_features if f not in output_df.columns]
        if missing_features:
            self._report_error("FEATURE_COMPLETENESS", f"Missing expected features: {missing_features}")
            return
    ...
    def _verify_performance(self, elapsed_time: float):
        ...
        if elapsed_time <= max_time:
            ...
            self._report_error("PERFORMANCE", f"Exceeded maximum time of {max_time} seconds: {elapsed_time:.2f}s")
