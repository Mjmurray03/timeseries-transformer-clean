...
Examples:
    ...
    parser.add_argument(
        ...
        default=",".join(
            getattr(Config, "DEFAULT_TICKERS", ["AAPL", "MSFT", "GOOGL", "NVDA", "TSLA"])
        ),
    )
    ...
    parser.add_argument(
        "--features", action="store_true", help="Enable feature engineering during download"
    )
    ...
    parser.add_argument(
        "--max-concurrent", type=int, default=5, help="Maximum concurrent downloads (default: 5)"
    )
...
class DataCollectionOrchestrator:
    ...
    def __init__(self, config: Optional[DataConfig] = None):
        ...
        self.collectors = {"yahoo": YahooFinanceCollector(self.config)}
            ...
            for ticker, data in validated_data.items():
                try:
                    ...
                    if self.engineer.validate_features(engineered_data):
                        ...
                        self.logger.debug(
                            f"Features engineered for {ticker} in {feature_time:.2f}s"
                        )
                    else:
    ...
    async def _save_datasets(self, datasets: Dict[str, pd.DataFrame], output_format: str) -> None:
        """
        ...
        try:
            ...
            for ticker, data in datasets.items():
                ...
                self.logger.debug(
                    f"Saved {len(data)} records for {ticker} in {output_format} format"
                )
    ...
    def _log_collection_summary(self, validation_results: Dict) -> None:
        ...
        if self.stats["feature_engineering_time"] > 0:
            self.logger.info(
                f"Feature engineering time: {self.stats['feature_engineering_time']:.2f}s"
            )
...
def print_header(
    tickers: List[str], years: int, output_dir: Path, features: bool, validation: bool
) -> None:
    """
    ...
    print("=" * 80)
    print(
        f"Tickers:           {len(tickers)} symbols ({', '.join(tickers[:5])}{', ...' if len(tickers) > 5 else ''})"
    )
...
async def main() -> None:
    ...
    try:
        ...
        print(
            f"[SUCCESS] Successful downloads: {stats['successful_downloads']}/{stats['tickers_requested']}"
        )
