...
class BaseValidator(ABC):
    ...
    def __init__(self, name: str, config: Optional[DataConfig] = None):
        self.name = name
        self.config = config
        self.stats = {"validations_performed": 0, "issues_found": 0, "last_validation": None}
    ...
    def _create_issue(
        self, severity: ValidationSeverity, message: str, column: Optional[str] = None, **kwargs
    ) -> ValidationIssue:
        """Create a validation issue."""
        return ValidationIssue(
            validator=self.name, severity=severity, message=message, column=column, **kwargs
        )
...
class SchemaValidator(BaseValidator):
    ...
    def validate(self, data: pd.DataFrame, ticker: str) -> List[ValidationIssue]:
        ...
        if data.empty:
            issues.append(self._create_issue(ValidationSeverity.CRITICAL, "DataFrame is empty"))
        ...
        if missing_columns:
            issues.append(
                self._create_issue(
                    ValidationSeverity.CRITICAL, f"Missing required columns: {missing_columns}"
                )
            )
        ...
        if unexpected_columns:
            issues.append(
                self._create_issue(
                    ValidationSeverity.WARNING, f"Unexpected columns found: {unexpected_columns}"
                )
            )
        ...
        if data.index.duplicated().any():
            ...
            issues.append(
                self._create_issue(
                    ValidationSeverity.ERROR, f"Found {duplicate_count} duplicate index values"
                )
            )
...
class RangeValidator(BaseValidator):
    ...
    def _validate_price_column(self, series: pd.Series, column: str) -> List[ValidationIssue]:
        ...
        if inf_count > 0:
            issues.append(
                self._create_issue(
                    ValidationSeverity.CRITICAL, f"Found {inf_count} infinite values", column=column
                )
            )
    ...
    def _validate_ohlc_relationships(self, data: pd.DataFrame) -> List[ValidationIssue]:
        ...
        try:
            ...
            if high_vs_open > 0:
                issues.append(
                    self._create_issue(
                        ValidationSeverity.ERROR, f"High < Open in {high_vs_open} rows"
                    )
                )
            ...
            if high_vs_close > 0:
                issues.append(
                    self._create_issue(
                        ValidationSeverity.ERROR, f"High < Close in {high_vs_close} rows"
                    )
                )
            ...
            if high_vs_low > 0:
                issues.append(
                    self._create_issue(
                        ValidationSeverity.CRITICAL, f"High < Low in {high_vs_low} rows"
                    )
                )
            ...
            if low_vs_open > 0:
                issues.append(
                    self._create_issue(
                        ValidationSeverity.ERROR, f"Low > Open in {low_vs_open} rows"
                    )
                )
            ...
            if low_vs_close > 0:
                issues.append(
                    self._create_issue(
                        ValidationSeverity.ERROR, f"Low > Close in {low_vs_close} rows"
                    )
                )
            ...
            issues.append(
                self._create_issue(
                    ValidationSeverity.WARNING, f"Error validating OHLC relationships: {e}"
                )
            )
    ...
    def _validate_temporal_consistency(self, index: pd.DatetimeIndex) -> List[ValidationIssue]:
        ...
        try:
            ...
            if future_dates > 0:
                issues.append(
                    self._create_issue(
                        ValidationSeverity.WARNING, f"Found {future_dates} future dates"
                    )
                )
            ...
            if len(weekend_dates) > 0:
                issues.append(
                    self._create_issue(
                        ValidationSeverity.INFO, f"Found {len(weekend_dates)} weekend dates"
                    )
                )
            ...
            if len(index) > 1:
                ...
                if large_gaps > 0:
                    issues.append(
                        self._create_issue(
                            ValidationSeverity.INFO, f"Found {large_gaps} large time gaps (>7 days)"
                        )
                    )
            ...
            issues.append(
                self._create_issue(
                    ValidationSeverity.WARNING, f"Error validating temporal consistency: {e}"
                )
            )
...
class CompletenessValidator(BaseValidator):
    ...
    def validate(self, data: pd.DataFrame, ticker: str) -> List[ValidationIssue]:
        ...
        if data.empty:
            issues.append(self._create_issue(ValidationSeverity.CRITICAL, "No data available"))
            self.stats["issues_found"] += len(issues)
            return issues
        ...
        for column in data.columns:
            ...
            if column_missing > 0:
                ...
                severity = (
                    ValidationSeverity.ERROR
                    if column_missing_ratio > self.max_missing_ratio
    ...
    def _check_consecutive_missing(self, data: pd.DataFrame) -> List[ValidationIssue]:
        ...
        try:
            ...
            issues.append(
                self._create_issue(
                    ValidationSeverity.WARNING, f"Error checking consecutive missing data: {e}"
                )
            )
    ...
    def _check_missing_patterns(self, data: pd.DataFrame) -> List[ValidationIssue]:
        ...
        try:
            ...
            issues.append(
                self._create_issue(
                    ValidationSeverity.WARNING, f"Error checking missing data patterns: {e}"
                )
            )
...
class DataValidator:
    ...
    def __init__(self, config: Optional[DataConfig] = None):
        ...
        # Overall statistics
        self.stats = {"validations_performed": 0, "total_issues_found": 0, "last_validation": None}
    ...
    def validate(self, data: pd.DataFrame, ticker: str) -> ValidationResult:
        ...
        result = ValidationResult(is_valid=is_valid, issues=all_issues, metadata=metadata)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {"overall": self.stats.copy(), "validators": validator_stats}
...
def validate_stock_data(
    data: pd.DataFrame, ticker: str, config: Optional[DataConfig] = None
) -> ValidationResult:
    """
