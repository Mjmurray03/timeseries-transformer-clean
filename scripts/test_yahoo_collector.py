...
async def main():
    ...
    config_dict = {
        ...
        "tickers": {"demo": ["AAPL", "MSFT"]},
        "date_ranges": {"recent": {"start_date": "2024-01-01", "end_date": "2024-01-31"}},
    }
    ...
    try:
        data = await collector.collect_ticker(
            "AAPL", start_date=date(2024, 1, 1), end_date=date(2024, 1, 31)
        )
    ...
    try:
        ...
        data_dict = await collector.collect_multiple(
            tickers, start_date=date(2024, 1, 1), end_date=date(2024, 1, 31), max_concurrent=2
        )
