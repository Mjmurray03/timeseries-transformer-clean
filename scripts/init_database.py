...
def create_metadata_database(db_path: str = "data/metadata/metadata.db"):
    ...
    try:
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_downloads_ticker_date ON data_downloads(ticker_id, start_date, end_date)"
        )
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_downloads_status ON data_downloads(status)")
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_quality_ticker_date ON data_quality_metrics(ticker_id, metric_date)"
        )
