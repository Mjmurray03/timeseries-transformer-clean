...
class BalancedDirectionalLoss(nn.Module):
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            self.class_counts[0] = (1 - self.smoothing) * self.class_counts[0] + self.smoothing * batch_down
            self.class_counts[1] = (1 - self.smoothing) * self.class_counts[1] + self.smoothing * batch_up
            self.total_samples = (1 - self.smoothing) * self.total_samples + self.smoothing * batch_total
