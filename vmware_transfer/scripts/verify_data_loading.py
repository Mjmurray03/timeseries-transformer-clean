...
class DataLoadingVerifier:
    ...
    def _verify_chronological_order(self, df: pd.DataFrame):
        ...
        if not isinstance(df.index, pd.DatetimeIndex):
            self._report_failure("CHRONOLOGICAL_ORDER_CHECK", "Cannot verify order without DatetimeIndex")
            return
    ...
    def _verify_trading_days(self, df: pd.DataFrame):
        ...
        if not isinstance(df.index, pd.DatetimeIndex):
            self._report_failure("TRADING_DAYS_CHECK", "Cannot verify trading days without DatetimeIndex")
            return
        ...
        if non_business_days:
            ...
            if large_gaps:
                print(f"⚠ Trading days check: Found {len(large_gaps)} gaps > 4 days (likely holidays)")
    ...
    def _verify_price_relationships(self, df: pd.DataFrame):
        ...
        if violations:
            self._report_failure("PRICE_RELATIONSHIP_CHECK", "Price relationship violations detected", violations)
        else:
